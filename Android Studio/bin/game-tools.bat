...
SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\@@product_vendor@@\AndroidStudioGameTools\studio%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\@@product_vendor@@\AndroidStudioGameTools\studio%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%APPDATA%\@@product_vendor@@\AndroidStudioGameTools\studio%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_STUDIO_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_STUDIO.hprof" -Didea.paths.selector=AndroidStudioGameTools %IDE_PROPERTIES_PROPERTY%
SET IDE_JVM_ARGS=-Didea.platform.prefix=AndroidStudio -Didea.jre.check=true -Didea.platform.prefix=AndroidStudioGameTools -Didea.load.plugins=false -Didea.initially.ask.config=force-not
