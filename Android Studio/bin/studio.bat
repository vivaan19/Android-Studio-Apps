...
SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\Google\AndroidStudio4.1\studio%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\Google\AndroidStudio4.1\studio%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%APPDATA%\Google\AndroidStudio4.1\studio%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_STUDIO_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_STUDIO.hprof" -Didea.paths.selector=AndroidStudio4.1 %IDE_PROPERTIES_PROPERTY%
